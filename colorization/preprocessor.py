...
class RemoteColorization:
    def __init__(self, grpc_address='localhost', grpc_port=9000, model_name='colorization', model_version=None):
        ...
        channel = grpc.insecure_channel(
            "{}:{}".format(self.grpc_address, self.grpc_port))
        ...
        assert self.color_coeff.shape == (
            313, 2), "Current shape of color coefficients does not match required shape"
    ...
    def __get_input_name_and_shape__(self):
        ...
        input_metadata, output_metadata = self.__get_input_and_output_meta_data__(
            result)
    ...
    def __preprocess_input__(self, original_frame):
        if original_frame.shape[2] > 1:
            frame = cv2.cvtColor(cv2.cvtColor(
                original_frame, cv2.COLOR_BGR2GRAY), cv2.COLOR_GRAY2RGB)
        else:
        ...
        img_l_rs = cv2.resize(
            img_lab.copy(), (self.input_width, self.input_height))[:, :, 0]
    ...
    def infer(self, original_frame):
        ...
        update_res = (res * self.color_coeff.transpose()
                      [:, :, np.newaxis, np.newaxis]).sum(1)
        ...
        img_bgr_out = np.clip(cv2.cvtColor(
            img_lab_out, cv2.COLOR_Lab2BGR), 0, 1)
...
def create_output_image(original_frame, img_bgr_out):
    ...
    colorize_image = (cv2.resize(img_bgr_out, im_show_size)
                      * 255).astype(np.uint8)
